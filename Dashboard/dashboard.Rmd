...
```{r top 5 crime VAR, message=F, warning=F}
# extract for tranforming into time series data
ts_AT <- covid19_AT %>% 
  dplyr::select(date, confirmed) %>%
  ts_xts()
...
```{r extract cases, message=F, warning=F}
# extract top 5 crime
top5crime <- seattle %>%
...
```{r top 5 crime VAR, message=FALSE, warning=F}
...
### Daily confirmed cases of COVID19 in Seattle
