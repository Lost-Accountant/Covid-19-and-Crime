...
autoplot(forecast$forecast$residuals) +
  ggtitle("Prediction on how many more covid19 cases compared to yesterday") +
  theme_classic() +
  ylab("Day-to-day change") +
  xlab(paste("Numebr of days since", common_time[1]))


### THE FOLLOWINGS ARE NOT FOR DISPLAY
...
# try to recreate daily count graph for prediction
# looks pretty ugly
# might not be a good idea.

prediction <- data.frame(cbind(diffinv(forecast$forecast$larceny_from_vehicle$upper[,2],
                            # 95% CI, not 80%
                            xi = top5crime_xts$larceny_from_vehicle[common_time[length(common_time)]]),
                    diffinv(forecast$forecast$larceny_from_vehicle$mean,
                            xi = top5crime_xts$larceny_from_vehicle[common_time[length(common_time)]]),
                    diffinv(forecast$forecast$larceny_from_vehicle$lower[,2],
                            xi = top5crime_xts$larceny_from_vehicle[common_time[length(common_time)]])))
colnames(prediction) <- c("upper", "mean", "lower")

prediction %>%
  ggplot(aes(x = as.numeric(row.names(prediction)))) +
  geom_line(aes(y=mean)) +
  geom_ribbon(aes(ymin = 0, ymax = upper), alpha = 0.5)
