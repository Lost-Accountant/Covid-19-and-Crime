...
boston %>%
  dplyr::select(y_month, MONTH, OFFENSE_DESCRIPTION, YEAR) %>%
  filter(OFFENSE_DESCRIPTION %in% boston_summary$OFFENSE_DESCRIPTION[1:5]) %>%
  count(YEAR, MONTH, OFFENSE_DESCRIPTION) %>%
  na.omit() %>%
  ggplot(aes(x=MONTH, y=n, group = YEAR, color = as.character(YEAR))) +
  geom_line() +
  facet_wrap(~OFFENSE_DESCRIPTION) +
  ggtitle("year to year comparison of the top 5 crime in boston in the past 5 years")
```
...
```
covid NOT significant to investigate person
...
```
property damage significant to covid, somehow
...
```
no causality. almost weak instantneous causality
...
covid significant to verbal dispute. verbal dispute significant to covid.

### Step 7: Impulse Response Function

Only properety damange and verbal dispute have granger causality.

```{r irf}
par(mfrow = c(1,2))
# investigate person
irf_damage_1 <- irf(VAR_damage, 
                         impulse = "property.damage", 
                         response = "residuals", 
                         n.ahead = 24)
irf_damage_2 <- irf(VAR_damage, 
                         impulse = "residuals", 
