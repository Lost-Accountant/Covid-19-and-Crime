...
dispute_diff_xts <- diff(dispute_xts)
...
An MA(1) model based on the auto.arima algorithm.
...
rownames(accu_compare) <- c("AR(3)", "Auto ARIMA", "Neural Net")
...
ann_forecast <- predict(dispute_ann, n.ahead = 24)

# plot forecast
ts.plot(test, ar_forecast$pred, 
        auto_forecast$pred, ann_forecast$mean,
        gpars = list(col = c("black", "red","green","blue")))
