...
# Philadelphia
```{r}
phil <- read.csv('/Users/zhuyihuang/Desktop/data/phil_crime/phil_crime.csv')
```
...
matplot(toPredict$time, 
        ...
        ylim = c(0.5, 5000), xlim = as.Date(c(covid19_PA$date[1], covid19_PA$date[length(covid19_PA$date)])))
...
VAR_offense <- VAR(y=as.ts(na.omit(combined_diff)[,c(1,6)]), p=3)
VAR_assault <- VAR(y=as.ts(na.omit(combined_diff)[,c(2,6)]), p=5)
VAR_vehicle <- VAR(y=as.ts(na.omit(combined_diff)[,c(3,6)]), p=3)
VAR_thefts <- VAR(y=as.ts(na.omit(combined_diff)[,c(4,6)]), p=6)
VAR_vandalism <- VAR(y=as.ts(na.omit(combined_diff)[,c(5,6)]), p=3)
```
...
```
COVID-19 may help to predict thefts from vehicle.
...
```
COVID-19 may help to predict thefts.
...
Only thefts from vehicle and thefts have granger causality.
...
# thefts
irf_thefts_1 <- irf(VAR_thefts, 
                         impulse = "thefts", 
                         response = "residuals", 
                         n.ahead = 24)
irf_thefts_2 <- irf(VAR_thefts, 
                         impulse = "residuals", 
                         response = "thefts", 
                         n.ahead = 24)
plot(irf_thefts_1)
