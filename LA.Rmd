...
matplot(toPredict$time, 
        exp(do.call(cbind, mgcv::predict.gam(gamCA$gam, toPredict, se.fit=TRUE)) %*% 
              Pmisc::ciMat()), 
        col='black', lty=c(1,2,2), type='l', xaxt='n', xlab='', ylab='count', 
        ylim = c(0.5, 5000), xlim = as.Date(c(covid19_CA$date[1], covid19_CA$date[length(covid19_CA$date)])))
...
VAR_battery <- VAR(y=as.ts(na.omit(combined_diff)[,c(1,6)]), p=2)
VAR_burglary <- VAR(y=as.ts(na.omit(combined_diff)[,c(2,6)]), p=6)
VAR_burglary_vehicle <- VAR(y=as.ts(na.omit(combined_diff)[,c(3,6)]), p=1)
VAR_vandalism <- VAR(y=as.ts(na.omit(combined_diff)[,c(4,6)]), p=1)
VAR_vehicle <- VAR(y=as.ts(na.omit(combined_diff)[,c(5,6)]), p=1)
```
...
Only 'battery - simple assault' have granger causality.
